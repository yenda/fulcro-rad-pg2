...
(defattr user-preferences :user/preferences :string
  {::attr/schema :production
   ::attr/identities #{:user/id}
   ::pg2/model->sql-value pr-str           ; Clojure map -> EDN string
   ::pg2/sql->model-value edn/read-string}) ; EDN string -> Clojure map
----
...
1. Use `::pg2/model->sql-value` and `::pg2/sql->model-value` transformers to convert between a supported type and your desired representation
