...
This document provides a comprehensive reference for the types and options supported by fulcro-rad-sql.
...
`:string`, `:password`, `:keyword`, `:symbol`, and `:enum` are all stored as `VARCHAR`. The default lengths vary by type but can be customized using the `::sql/max-length` option.
...
(defattr user-name :user/name :string
  {::attr/schema :production
   ...
   ::sql/max-length 100})  ; VARCHAR(100) instead of default VARCHAR(2048)
----
...
These options are defined in the `com.fulcrologic.rad.database-adapters.sql` namespace.
...
|`::sql/owns-ref?`
|Indicates the FK column is stored on this entity's table (for one-to-one relationships)
|To-one `:ref` attributes

|`::sql/ref`
|Specifies the reverse FK attribute (FK is on the target table)
|To-many `:ref` attributes

|`::sql/delete-referent?`
|When a reference changes, delete the orphaned entity (simulates Datomic's `isComponent`)
|`:ref` attributes
...
|`::sql/model->sql-value`
|Custom transformer function `(fn [clj-value] sql-value)` for writing
|Any attribute

|`::sql/sql->model-value`
|Custom transformer function `(fn [sql-value] clj-value)` for reading
|Any attribute
|===
...
(defattr user-id :user/id :uuid
  {::attr/identity? true
   ::attr/schema :production
   ::sql/table "app_users"})  ; Custom table name

(defattr user-created :user/created-at :instant
  {::attr/schema :production
   ::attr/identities #{:user/id}
   ::sql/column-name "created_timestamp"})  ; Custom column name
----
...
(defattr order-line-items :order/line-items :ref
  {::attr/cardinality :many
   ...
   ::attr/schema :production
   ::attr/identities #{:order/id}
   ::sql/ref :line-item/order})  ; Points to reverse ref attribute
...
(defattr user-preferences :user/preferences :string
  {::attr/schema :production
   ::attr/identities #{:user/id}
   ::sql/model->sql-value pr-str           ; Clojure map -> EDN string
   ::sql/sql->model-value edn/read-string}) ; EDN string -> Clojure map
----
...
|Document
|`:uuid`, `:string`, `:ref`
|`table`, `owns-ref?`, `delete-referent?`
...
The following PostgreSQL types are NOT currently supported by fulcro-rad-sql:
...
1. Use `::sql/model->sql-value` and `::sql/sql->model-value` transformers to convert between a supported type and your desired representation
