...
status: stable
contributions:
  - Add examples for custom type transformers
  - Document pg2 type encoder/decoder configuration
  - Add migration guide from JDBC type handling
---
...
form->sql-value (write.clj)
    ...
    ├─ custom ::pg2/form->sql-value → apply transformer
...
IMPORTANT: Write transformers (`::pg2/form->sql-value`) are applied automatically during save operations. However, read transformers (`::pg2/sql->form-value`) are NOT automatically applied. If you need custom deserialization, you must handle it in a custom resolver or manually apply the transformer.
...
|SQL
|Manual via `::pg2/delete-orphan?` option (requires `::pg2/fk-attr`)
|===
...
(defattr app-config :app/config :string
  {::attr/schema :production
   ...
   ::pg2/form->sql-value j/write-value-as-string
   ;; Read: JSON string → Clojure map
   ::pg2/sql->form-value j/read-value})
----
...
----
;; From write.clj
(defn error-condition [^PGErrorResponse e]
  (case (.getCode e)
...
│  ::pg2/form->sql-value = (fn [v] ...)  ; optional          │
│  ::pg2/sql->form-value = (fn [v] ...)  ; optional          │
└─────────────────────────────────────────────────────────────┘
                              │
        ┌─────────────────────┴─────────────────────┐
        ▼                                           ▼
...
3. **Write transformers auto-apply:** `::pg2/form->sql-value` is called during saves
4. **Read transformers don't auto-apply:** `::pg2/sql->form-value` must be handled manually
