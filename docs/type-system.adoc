...
form->sql-value (write.clj)
    ...
    ├─ custom ::pg2/model->sql-value → apply transformer
...
IMPORTANT: Write transformers (`::pg2/model->sql-value`) are applied automatically during save operations. However, read transformers (`::pg2/sql->model-value`) are NOT automatically applied. If you need custom deserialization, you must handle it in a custom resolver or manually apply the transformer.
...
(defattr app-config :app/config :string
  {::attr/schema :production
   ...
   ::pg2/model->sql-value j/write-value-as-string
   ;; Read: JSON string → Clojure map
   ::pg2/sql->model-value j/read-value})
----
...
│  ::pg2/model->sql-value = (fn [v] ...)  ; optional         │
│  ::pg2/sql->model-value = (fn [v] ...)  ; optional         │
└─────────────────────────────────────────────────────────────┘
                              │
        ┌─────────────────────┴─────────────────────┐
        ▼                                           ▼
...
3. **Write transformers auto-apply:** `::pg2/model->sql-value` is called during saves
4. **Read transformers don't auto-apply:** `::pg2/sql->model-value` must be handled manually
