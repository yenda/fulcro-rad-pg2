(ns com.fulcrologic.rad.database-adapters.sql.ref-lifecycle-test
...
;;
;; Pattern: document/metadata has `ref :metadata/document` and `delete-referent?`
      ...
      (fn [conn env]
        ...
        (let [doc-tempid (tempid/tempid)
          ...
          (let [new-meta-tempid (tempid/tempid)
            ...
            ;; Old metadata should be deleted (delete-referent? true)
    ...
    (with-test-db
      (fn [conn env]
        ...
        (let [acct-tempid (tempid/tempid)
          ...
          ;; Note: account/addresses doesn't have delete-referent?, so removing
          ...
          (let [remove-delta {[:account/id acct-id]
            ...
            ;; Both addresses should still exist (no delete-referent?)
            (is (= 2 (count-rows conn "addresses")) "Addresses should still exist (no delete-referent?)")
...
(deftest ^:integration self-ref-remove-parent-orphans-child
  (testing "Remove parent reference, child becomes orphan (no delete-referent? on parent attr)"
    (with-test-db
      (fn [conn env]
        ...
        (let [parent-tempid (tempid/tempid)
          ...
          (let [remove-delta {[:category/id child-id]
            ...
            ;; Both categories should still exist (no delete-referent? on category/parent)
