(ns com.fulcrologic.rad.database-adapters.pg2.write
...
(defn form->sql-value
  ...
  [{::attr/keys [type cardinality]
    ::rad.pg2/keys [form->sql-value]} form-value]
  (cond
     ...
     (reduce
      (fn [acc [attr-k {:keys [before after]}]]
        (let [{::attr/keys [_type cardinality]
               ::rad.pg2/keys [fk-attr delete-orphan?]} (key->attribute attr-k)]
  ...
  (let [{::attr/keys [type cardinality schema] :as attr} (k->a k)]
    (when (= schema target-schema)
      (cond
        ...
        (= cardinality :one) (when-not (::rad.pg2/fk-attr attr) attr)
        :else attr))))
...
(mx/defn save-form! :- [:map [:tempids :map]]
  ...
  [env :- [:map
           ...
           [::rad.pg2/connection-pools [:map-of :keyword :some]]]
   params :- [:map [::rad.form/delta :map]]]
  (let [{::attr/keys [key->attribute]
         ::rad.pg2/keys [connection-pools]} env
